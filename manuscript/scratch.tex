...
Additive genetic variance in relative fitness, $V_A$, is simultaneously the primary substrate for adaptive evolution and a mechanism of accelerated genetic drift. Under Fisher's Fundamental Theorem of Natural Selection, the partial change in mean fitness attributable to the action of selection equals $V_A/\bar{w}$ per generation, establishing heritable fitness variance as the immediate currency of adaptive change (Fisher 1930; Price 1972). Measuring $V_A$ is therefore essential for predicting the rate and trajectory of adaptation, characterizing the limits of evolutionary rescue under novel environmental pressures, and understanding the processes that maintain quantitative variation in nature (Crow 1958; Lande 1982). Yet $V_A$ is not merely the engine of directional adaptation: in a finite population, heritable variance in reproductive success generates a systematic inflation of family-size variance beyond the neutral Poisson expectation, accelerating allele-frequency change at neutral sites and depressing the effective population size, $N_e$, below the census size, $N$. These two consequences—accelerated adaptation and accelerated genetic drift—are inseparable features of the same underlying quantity, and empirical estimates of $V_A$ therefore simultaneously constrain the potential for evolutionary rescue and the expected level of genome-wide neutral diversity.

Historically, estimation of $V_A$ in natural populations was severely limited by the confounding of heritable and environmental sources of phenotypic variance. The development of the animal model—a mixed-effects framework that partitions phenotypic variance using known pedigree relationships—enabled direct estimation of quantitative genetic parameters in wild, unmanipulated populations (Kruuk 2004; Wilson et al. 2010). Deployment of this approach across long-term ecological studies has generated a substantial body of empirical $V_A$ estimates for fitness and fitness-related traits. Syntheses of these datasets consistently report abundant heritable variance in relative fitness across a broad diversity of taxa, with estimates based on lifetime reproductive success or analogous demographic proxies typically falling between 0.1 and 0.3 on the relative fitness scale (Postma 2014; Bonnet et al. 2022). Several sources of systematic upward bias in animal-model estimates are well-documented: unweighted lifetime reproductive success overestimates the additive variance of Fisherian fitness by failing to discount delayed reproduction in age-structured populations (Sæther and Engen 2015; de Villemereuil et al. 2020); and the informationally sparse pedigrees characteristic of monitored wild populations lack the statistical power to partition additive from dominance and epistatic contributions cleanly, allowing non-additive variance to inflate the estimated $V_A$ (Wolak and Keller 2014). Nonetheless, even if only a modest fraction of the reported variance is genuinely additive and transmissible, the implied reproductive overdispersion substantially exceeds the demographic stability of the neutral Wright-Fisher expectation—with consequences that propagate directly into the neutral coalescent.

In a finite population of constant census size $N$, the neutral expectation is that variance in family size, $V_k$, is driven entirely by stochastic, non-heritable environmental noise, approaching the Poisson expectation $V_k \approx 2$ for a diploid randomly mating population (Wright 1938). This baseline yields $N_e \approx N$ and governs the expected level of genome-wide neutral diversity. The Robertson effect (Robertson 1961) arises when this neutral baseline is violated: when reproductive success has an additive genetic basis, successful parents disproportionately transmit advantageous alleles to their offspring, generating a positive temporal autocovariance in fitness across generations (Buffalo and Coop 2019). Because census size is bounded, the sustained reproductive expansion of high-fitness lineages necessarily entails the accelerated stochastic loss of low-fitness lineages, inflating $V_k$ and compounding this effect across generations. The consequence is a systematic reduction in $N_e$ relative to $N$ and an accompanying erosion of genome-wide neutral diversity. At sufficiently large $V_A$, allele-frequency dynamics at neutral sites are no longer governed primarily by demographic stochasticity but by selective interference—the stochastic noise generated in the frequency trajectories of neutral alleles by heritable fitness differences among lineages (Gillespie 2000; Sella and Barton 2019)—and the effective timescale of the neutral coalescent is compressed accordingly.

Translating this demographic consequence of $V_A$ into a quantitative prediction for neutral nucleotide diversity requires integrating selective interference across the full physical architecture of the genome. Classical background selection theory has predominantly modeled the constraint imposed by closely linked selected loci, where the per-generation decay of statistical association between a neutral site and a selected background is far slower than the one-half per generation guaranteed by independent assortment (Charlesworth et al. 1993; Hudson and Kaplan 1995). A common implicit assumption of this tradition is that independent assortment defines an effective ceiling beyond which selective constraint on neutral diversity asymptotically vanishes. This assumption is incorrect: heritable fitness variance distributed across the genome imposes a systematic genome-wide constraint on neutral diversity through the Robertson effect, even when all selected loci assort independently of the focal neutral site (Matheson and Masel 2025). For the magnitudes of $V_A$ reported in pedigree studies of wild populations, this unlinked component alone predicts substantial reductions in $N_e/N$ before any contribution from physical linkage is considered. Standard background selection models, by conditioning on tight physical linkage and assuming random mating, systematically understate this genome-wide baseline and consequently misrepresent the total selective load on the neutral coalescent.

Rigorously evaluating the total evolutionary constraint imposed by large empirical $V_A$ estimates therefore requires an integrated framework that extends standard background selection models in three directions. First, the social mating system of the study population must be incorporated explicitly: under strict monogamy, paired males and females share completely correlated reproductive outcomes, concentrating the heritable component of family-size variance across both parental gametes simultaneously and amplifying the Robertson-effect penalty relative to a randomly mating population; increasing rates of extra-pair paternity progressively dilute this concentration. Second, the linked contribution of selective interference must be integrated continuously across empirical recombination maps using the quantitative genetics framework of Santiago and Caballero (1995, 1998), which tracks the multi-generational persistence of statistical associations between neutral sites and the selected genetic backgrounds along their chromosomes. Third, the distribution of heritable fitness variance across the genome must reflect empirical functional target sizes derived from genome annotation rather than heuristic physical approximations. The resulting predictions depend on the assumed genetic architecture of fitness: we show formally via Jensen's inequality that the infinitesimal model—which distributes selective variance uniformly across all functional targets—is conservative in expectation for randomly positioned loci of large effect, so that predictions derived under this assumption represent upper bounds on $N_e/N$. Whether this conservatism holds when large-effect loci are enriched in regions of reduced recombination, as empirical evidence in vertebrates suggests, depends on the specific genomic architecture of fitness and is evaluated analytically.

In this study, we parameterize this integrated framework with empirical $V_A$ estimates from pedigree-based animal models of monitored wild vertebrate populations. We first derive analytical expressions for the mating-system-corrected Robertson effect, establishing the genome-wide unlinked baseline reduction in $N_e/N$ as a function of heritable fitness variance and extra-pair paternity rate. We then apply the Santiago-Caballero spatial framework with empirical genetic map lengths and annotation-based functional target sizes to compute closed-form chromosome-level predictions for the reduction in effective size due to physical linkage, and define the Relative Linkage Penalty—the fraction of neutral diversity eroded by genomic architecture above the unlinked baseline. We further derive a diagnostic upper bound, $V_{A,\text{max}}$, specifying the maximum additive genetic variance in relative fitness compatible with maintaining a given minimum fraction of neutral diversity, and use it to evaluate the theoretical consistency of published animal-model estimates. Applying these predictions to published $V_A$ estimates alongside empirical genome-wide nucleotide diversity data, we demonstrate that the reported magnitudes imply a severe depletion of neutral diversity far in excess of what is observed in wild vertebrate populations, which empirically maintain genome-wide $\pi$ typically above $10^{-3}$ (Leffler et al. 2012; Buffalo 2021). This structural mismatch between ecologically measured variance and coalescent-scale diversity points toward one or more of three resolutions: pedigree-based animal models capture non-additive or environmentally structured covariance among relatives—including spatially autocorrelated environments or indirect genetic effects from kin-structured social contexts—rather than purely transmissible additive variance (Stopher et al. 2012; Baud et al. 2022); the effective breeding population substantially exceeds the monitored census through cryptic immigration or population structure; or the selective environment fluctuates sufficiently rapidly that high-fitness lineages are prevented from expanding long enough to erode coalescent diversity before selection reverses (Wittmann et al. 2017; Buffalo and Coop 2020).

\section{Theory and Model}
\subsection{Genome-wide Effect of Heritable Variance in Fitness}
To isolate the genome-wide consequences of heritable fitness variance, we first derive the expected reduction in neutral diversity for a locus unlinked to all selected sites—the unlinked baseline. In a standard neutral, diploid Wright-Fisher population of constant census size $N$, variance in offspring number is Poisson distributed ($V_k \approx \bar{k} = 2$), yielding $N_e \approx N$ (Wright 1938; Crow and Kimura 1970). This baseline explicitly assumes that variance in reproductive success is driven entirely by non-heritable, stochastic environmental noise.

The Robertson effect (Robertson 1961) emerges when this assumption is violated by additive genetic variance in relative fitness, $V_A$. Because $V_A$ represents transmissible differences in reproductive success—encompassing both the expansion of high-fitness lineages and the continuous, deterministic purging of deleterious load that drives background selection—it generates a positive temporal autocovariance in fitness across generations (Buffalo and Coop 2019). Bounded by a constant $N$, the sustained multi-generational variance in lineage survival continuously inflates $V_k$, exacerbating genetic drift.

For a focal neutral allele assorting independently of all selected loci, the initial variance in allele frequency induced by this selective interference is proportional to $V_A$. Mendelian segregation decays the statistical association between the neutral allele and its specific selective background by exactly one-half each generation. Summing this geometric series of selective-variance contributions alongside the standard neutral sampling variance (Appendix A) yields the long-term expected reduction in effective size under complete independent assortment:

\begin{equation}
    \frac{N_e}{N} = \frac{1}{1 + 4 V_A}
    \label{unlinked_baseline}
\end{equation}

Because this derivation depends strictly on the halving of statistical associations via free recombination, it requires no assumption regarding the temporal stationarity of the selective background. Consequently, \autoref{unlinked_baseline} applies universally regardless of the selective regime maintaining $V_A$—encompassing mutation-selection-drift balance, episodic selective sweeps, or fluctuating selection. Crucially, this unlinked baseline establishes that physical linkage is not a prerequisite for selective interference; heritable fitness variance inherently inflates $V_k$, exerting a systematic, genome-wide suppression of $N_e$ strictly through the heritable overdispersion of family sizes. This unlinked reduction in effective size directly compresses the neutral coalescent, lowering the baseline expectation for genome-wide nucleotide diversity before the effects of physical chromosomal architecture are considered.
